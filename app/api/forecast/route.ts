...
export async function GET(request: Request) {
  ...
  try {
    ...
    const apiUrl = `https://api.openweathermap.org/data/2.5/forecast?q=${encodeURIComponent(city)}&units=metric&appid=${apiKey!.substring(0, 3)}...&lang=es`
    ...
    return NextResponse.json(
      {
        error: "No se pudieron obtener los datos del pronóstico. Intenta nuevamente más tarde.",
      },
      { status: 500 },
    )
  }
