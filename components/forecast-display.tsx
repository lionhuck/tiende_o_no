...
export default function ForecastDisplay({ forecast }: ForecastDisplayProps) {
  ...
  const groupByDay = () => {
    ...
    forecast.list.forEach((item: any) => {
      ...
      // Use date only (without time) as the key
    ...
    // Sort each day's forecasts by time
  ...
  // Get day name (Hoy, Mañana, or weekday)
  const getDayName = (dateStr: string) => {
    ...
    if (date.getTime() === today.getTime()) {
      ...
      return new Date(dateStr).toLocaleDateString("es-ES", { weekday: "long", day: "numeric" })
    }
  }
  ...
  const formatWarnings = (warnings: string[]) => {
    ...
    // Eliminamos la verificación específica para "Noche"
